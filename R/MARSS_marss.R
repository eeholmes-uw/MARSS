...
#'
#' @description
#' The form of MARSS models for users is "marxss", the MARSS models with
#' inputs. See [MARSS.marxss()]. In the internal algorithms (e.g.
#' [MARSSkem()]), the "marss" form is used and the
#' \eqn{\mathbf{D}\mathbf{d}_t}{Dd(t)} are incorporated into the
#' \eqn{\mathbf{a}_t}{a(t)} matrix and \eqn{\mathbf{C}\mathbf{c}_t}{Cc(t)}
#' are incorporated into the \eqn{\mathbf{u}_t}{u(t)}. The
#' \eqn{\mathbf{a}}{a} and \eqn{\mathbf{u}}{u} matrices then become
#' time-varying if the model includes \eqn{\mathbf{d}_t}{d(t)} and
#' \eqn{\mathbf{c}_t}{c(t)}.
#'
...
#' estimated parameter not a diffuse prior. To remove clutter, the rest of
#' the parameters are shown as time-constant (no \eqn{t} subscript) but all
#' parameters can be time-varying.
#'
#' Note, "marss" is a model form. A model form is defined by a collection of
#' form functions discussed in [marssMODEL]. These functions are not exported
#' to the user, but are called by [MARSS()] using the argument `form`. These
#' internal functions convert the users model list into the vec form of a
#' MARSS model and do extensive error-checking.
#'
#' @section Usage:
#' ```
#' MARSS(y,
...
#'
#' @seealso [marssMODEL], [MARSS.marxss()]
#'
#' @examples
#' \dontrun{
