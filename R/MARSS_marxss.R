...
#'
#' @description
#' The argument `form="marxss"` in a [MARSS()] function call specifies a
#' MAR-1 model with eXogenous variables model. This is a MARSS(1) model of
#' the form:
...
#' Note, by default \eqn{\mathbf{V0}}{V0} is a matrix of all zeros and thus
#' \eqn{\mathbf{x}_1}{x(1)} or \eqn{\mathbf{x}_0}{x(0)} is treated as an
#' estimated parameter not a diffuse prior.
#'
#' Note, "marxss" is a model form. A model form is defined by a collection of
#' form functions discussed in [marssMODEL]. These functions are not exported
#' to the user, but are called by [MARSS()] using the argument `form`.
#'
#' @section Usage:
#' ```
#' MARSS(y,
...
#'
#' @details
#' The allowed arguments when `form="marxss"` are 1) the arguments common to
#' all forms: "y" (data), "inits", "control", "method", "form", "fit",
#' "silent", "fun.kf" (see [MARSS()] for information on these arguments) and
#' 2) the argument "model" which is a list describing the MARXSS model (the
#' model list is described below). See the
...
#' for examples.
#'
#' The argument `model` must be a list. The elements in the list specify the
#' structure for the \eqn{\mathbf{B}}{B}, \eqn{\mathbf{u}}{u},
#' \eqn{\mathbf{C}}{C}, \eqn{\mathbf{c}}{c}, \eqn{\mathbf{Q}}{Q},
#' \eqn{\mathbf{Z}}{Z}, \eqn{\mathbf{a}}{a}, \eqn{\mathbf{D}}{D},
#' \eqn{\mathbf{d}}{d}, \eqn{\mathbf{R}}{R}, \eqn{\mathbf{x}_0}{x0}, and
#' \eqn{\mathbf{V}_0}{V0} in the MARXSS model (above). The list elements can
#' have the following values:
#'
#' * `Z` Default="identity". A text string, "identity","unconstrained",
#'   "diagonal and unequal", "diagonal and equal", "equalvarcov", or
#'   "onestate", or a length n vector of factors specifying which of the m
#'   hidden state time series correspond to which of the n observation time
#'   series. May be specified as a n x m list matrix for general specification
#'   of both fixed and shared elements within the matrix. May also be
#'   specified as a numeric n x m matrix to use a custom fixed
#'   \eqn{\mathbf{Z}}{Z}. "onestate" gives a n x 1 matrix of 1s.
#'   "identity","unconstrained", "diagonal and unequal", "diagonal and equal",
#'   and "equalvarcov" all specify n x n matrices.
#' * `B` Default="identity". A text string, "identity", "unconstrained",
#'   "diagonal and unequal", "diagonal and equal", "equalvarcov", "zero". Can
#'   also be specified as a list matrix for general specification of both fixed
#'   and shared elements within the matrix. May also be specified as a numeric
#'   m x m matrix to use custom fixed \eqn{\mathbf{B}}{B}, but in this case
#'   all the eigenvalues of \eqn{\mathbf{B}}{B} must fall in the unit circle.
#' * `U`, `x0` Default="unconstrained". A text string, "unconstrained",
#'   "equal", "unequal" or "zero". May be specified as a m x 1 list matrix
#'   for general specification of both fixed and shared elements within the
#'   matrix. May also be specified as a numeric m x 1 matrix to use a custom
#'   fixed \eqn{\mathbf{u}}{u} or \eqn{\mathbf{x}_0}{x(0)}. Notice that `U`
#'   is capitalized in the `model` argument and output lists.
#' * `A` Default="scaling". A text string, "scaling","unconstrained", "equal",
#'   "unequal" or "zero". May be specified as a n x 1 list matrix for general
#'   specification of both fixed and shared elements within the matrix. May
#'   also be specified as a numeric n x 1 matrix to use a custom fixed
#'   \eqn{\mathbf{a}}{a}. Care must be taken when specifying `A` so that the
#'   model is not under-constrained and unsolvable. The default, "scaling",
#'   only applies to \eqn{\mathbf{Z}}{Z} matrices that are design matrices
#'   (only 1s and 0s and all rows sum to 1). When a column in
#'   \eqn{\mathbf{Z}}{Z} has multiple 1s, the first row in the
#'   \eqn{\mathbf{a}}{a} matrix associated with those \eqn{\mathbf{Z}}{Z}
#'   rows is 0 and the other associated \eqn{\mathbf{a}}{a} rows have an
#'   estimated value. This is used to treat \eqn{\mathbf{a}}{a} as an
#'   intercept where one intercept for each \eqn{\mathbf{x}}{x} (hidden
#'   state) is fixed at 0 and any other intercepts associated with that
#'   \eqn{\mathbf{x}}{x} have an estimated intercept. This ensures a solvable
#'   model when \eqn{\mathbf{Z}}{Z} is a design matrix. Note in the model
#'   argument and output, `A` is capitalized.
#' * `Q` Default="diagonal and unequal". A text string, "identity",
#'   "unconstrained", "diagonal and unequal", "diagonal and equal",
#'   "equalvarcov", "zero". May be specified as a list matrix for general
#'   specification of both fixed and shared elements within the matrix. May
#'   also be specified as a numeric g x g matrix to use a custom fixed matrix.
#'   Default value of g is m, so \eqn{\mathbf{Q}}{Q} is a m x m matrix. g is
#'   the number of columns in \eqn{\mathbf{G}}{G} (below).
#' * `R` Default="diagonal and equal". A text string, "identity",
#'   "unconstrained", "diagonal and unequal", "diagonal and equal",
#'   "equalvarcov", "zero". May be specified as a list matrix for general
#'   specification of both fixed and shared elements within the matrix. May
#'   also be specified as a numeric h x h matrix to use a custom fixed matrix.
#'   Default value of h is n, so \eqn{\mathbf{R}}{R} is a n x n matrix. h is
#'   the number of columns in \eqn{\mathbf{H}}{H} (below).
#' * `V0` Default="zero". A text string, "identity", "unconstrained",
#'   "diagonal and unequal", "diagonal and equal", "equalvarcov", "zero". May
#'   be specified as a list matrix for general specification of both fixed and
#'   shared elements within the matrix. May also be specified as a numeric m x
#'   m matrix to use a custom fixed matrix.
#' * `D` and `C` Default="zero". A text string, "identity", "unconstrained",
#'   "diagonal and unequal", "diagonal and equal", "equalvarcov", "zero". Can
#'   be specified as a list matrix for general specification of both fixed and
#'   shared elements within the matrix. May also be specified as a numeric
#'   matrix to use custom fixed values. Must have n rows
#'   (\eqn{\mathbf{D}}{D}) or m rows (\eqn{\mathbf{C}}{C}).
#' * `d` and `c` Default="zero". Numeric matrix. No missing values allowed.
#'   Must have 1 column or the same number of columns as the data,
#'   \eqn{\mathbf{y}}{y}. The numbers of rows in \eqn{\mathbf{d}}{d} must be
#'   the same as number of columns in \eqn{\mathbf{D}}{D}; similarly for
#'   \eqn{\mathbf{c}}{c} and \eqn{\mathbf{C}}{C}.
#' * `G` and `H` Default="identity". A text string, "identity". Can be
#'   specified as a numeric matrix or array for time-varying cases. Must have
#'   m rows and g columns (\eqn{\mathbf{G}}{G}) or n rows and h columns
#'   (\eqn{\mathbf{H}}{H}). g is the dim of \eqn{\mathbf{Q}}{Q} and h is the
#'   dim of \eqn{\mathbf{R}}{R}.
#' * `tinitx` Default=0. Whether the initial state is specified at t=0
#'   (default) or t=1.
#'
#' All parameters except \eqn{\mathbf{x}_0}{x0} and \eqn{\mathbf{V}_0}{V0}
#' may be time-varying. If time-varying, then text shortcuts cannot be used.
#' Enter as an array with the 3rd dimension being time. Time dimension must be
#' 1 or equal to the number of time-steps in the data. See Quick Start guide
#' (`RShowDoc("Quick_Start",package="MARSS")`) or the User Guide
#' (`RShowDoc("UserGuide",package="MARSS")`) for examples.
#'
#' Valid model structures for `method="BFGS"` are the same as for
#' `method="kem"`. See [MARSSoptim()] for the allowed options for this method.
#'
#' The default estimation method, `method="kem"`, is the EM algorithm
#' described in the MARSS User Guide. The default settings for the control and
#' inits arguments are set via `MARSS:::alldefaults$kem` in
#' `MARSSsettings.R`. The defaults for the model argument are set in
#' `MARSS_marxss.R`. For this method, they are:
#'
#' * inits = list(B=1, U=0, Q=0.05, Z=1, A=0, R=0.05, x0=-99, V0=0.05,
#'   G=0, H=0, L=0, C=0, D=0, c=0, d=0)
#' * model = list(Z="identity", A="scaling", R="diagonal and equal",
#'   B="identity", U="unconstrained", Q="diagonal and unequal",
#'   x0="unconstrained", V0="zero", C="zero", D="zero",
#'   c=matrix(0,0,1), d=matrix(0,0,1), tinitx=0, diffuse=FALSE)
...
#'   demean.states=FALSE). You can read about these in [MARSS()]. If you want
#'   to speed up your fits, you can turn off most of the model checking using
#'   `trace=-1`.
#' * fun.kf = "MARSSkfas". This sets the Kalman filter function to use.
#'   `MARSSkfas()` is generally more stable as it uses Durban & Koopman's
#'   algorithm. But it may dramatically slow down when the data set is large
#'   (more than 10 rows of data). Try the classic Kalman filter algorithm to
#'   see if it runs faster by setting `fun.kf="MARSSkfss"`. You can read about
#'   the two algorithms in [MARSSkf()].
#'
...
#' of confidence intervals and bias. See [plot.marssMLE()] for some default
#' plots of a model fit.
#'
#' @author
#' Eli Holmes, NOAA, Seattle, USA.
#'
#' @seealso [marssMODEL], [MARSS.dfa()]
#'
#' @examples
#' \dontrun{
